scripts/krb-longrun.sh run --name g1-1 -- uv run -m actuate.experiment --generations 10 --generation-duration 2h --base-port 8185 --tmux-prefix claude-1 --world-dir worlds/mujoco-g1 --template worlds/mujoco-g1/template/agent_no_soul --model claude-fable-5 --claude-effort medium --no-claude-native-sandbox --env-file .env

# Notes for this DDS world:
#
...
#
# Setup (one-time, per machine): the world runs in the `g1` conda
...
#   fastapi + uvicorn for the world's HTTP lifecycle, h5py for recordings, and
#   viser/mjviser for the spectator:
#     uv pip install --python "$(conda info --base)/envs/g1/bin/python" fastapi uvicorn websockets h5py viser mjviser
