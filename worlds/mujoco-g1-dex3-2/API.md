...
This `-2` world uses a free root body. The root pose is not actuator-controlled:
the agent moves the robot by sending torques/targets to the 43 robot actuators
and relying on physics and ground contact. The extra root coordinates appear in
`state.qpos`/`state.qvel`, but they do not have matching `ctrl` entries.
...
|---------------|---------------------------------|
| root freejoint | `qpos[0:7]`, `qvel[0:6]` |
| `ctrl[0:6]` | `qpos[7:13]`, `qvel[6:12]` |
| `ctrl[6:12]` | `qpos[13:19]`, `qvel[12:18]` |
| `ctrl[12:15]` | `qpos[19:22]`, `qvel[18:21]` |
| `ctrl[15:22]` | `qpos[22:29]`, `qvel[21:28]` |
| `ctrl[22:29]` | `qpos[36:43]`, `qvel[35:42]` |
| `ctrl[29:36]` | `qpos[29:36]`, `qvel[28:35]` |
| `ctrl[36:43]` | `qpos[43:50]`, `qvel[42:49]` |

In particular, the right arm is controlled by `ctrl[22:29]`, but its joint
state is `qpos[36:43]` and `qvel[35:42]`. The left hand sits between the left
and right arms in `qpos` order, so `qpos[29:36]` is the left hand, not the
right arm. The first 7 `qpos` entries and first 6 `qvel` entries are the
passive root freejoint. For robust code, read `model.actuators[*].target` and
the matching joint metadata from `/observe` instead of hard-coding equal
indices.
